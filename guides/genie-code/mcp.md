...
### 방법 2: `.mcp_servers.json` 파일로 설정 (프로그래밍 방식)

Databricks Workspace에서 `.assistant/.mcp_servers.json` 파일을 직접 편집하여 MCP 서버를 구성할 수 있습니다. 이 방식은 팀 전체에 동일한 MCP 설정을 배포하거나, 버전 관리가 필요한 경우에 유용합니다.

**파일 위치**: Workspace 파일 브라우저에서 `.assistant/.mcp_servers.json`

{% hint style="info" %}
이 파일은 Workspace 루트의 `.assistant` 폴더 안에 위치합니다. 파일이 없으면 직접 생성하면 됩니다. `skills` 폴더와 같은 레벨에 있습니다.
{% endhint %}
