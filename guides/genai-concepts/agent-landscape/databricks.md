...
│                  사용자 / 애플리케이션               │
├─────────────────────────────────────────────────┤
│  Databricks Apps │ Genie Code │ Genie Space     │  ← 인터페이스
├─────────────────────────────────────────────────┤
│  Agent Bricks (KA, Genie Agent, Supervisor)     │  ← 노코드 빌드
│  Agent Framework (ChatAgent, LangGraph 통합)    │  ← 프로코드 빌드
├─────────────────────────────────────────────────┤
│  Mosaic AI Gateway (멀티모델 라우팅, Fallback)   │  ← 모델 관리
│  Model Serving (서버리스, 자동 스케일링)           │  ← 배포
├─────────────────────────────────────────────────┤
│  MLflow 3.0 (Tracing, Evaluate, Monitoring)     │  ← 관측
├─────────────────────────────────────────────────┤
│  Unity Catalog (데이터 + 모델 + Agent 거버넌스)   │  ← 거버넌스
│  Vector Search │ Delta Lake │ Lakebase          │  ← 데이터
└─────────────────────────────────────────────────┘
```
...
- [Agent 프레임워크 생태계](../agent-frameworks.md): LangGraph, CrewAI 등과의 비교
- [Agent UI & 배포](../agent-ui-stack.md): Streamlit, Databricks Apps로 Agent 배포
