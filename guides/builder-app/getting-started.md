...
- `LAKEBASE_*`: 프로젝트/대화 이력을 저장할 PostgreSQL DB. 미설정 시 로컬 파일 시스템만 사용
{% endhint %}
...
{% hint style="warning" %}
Lakebase 없이도 Builder App을 실행할 수 있습니다. 이 경우 프로젝트 데이터는 로컬 파일 시스템(`projects/` 디렉토리)에만 저장됩니다.
{% endhint %}
...
{% hint style="tip" %}
에이전트가 사용하는 도구 호출 과정이 실시간으로 스트리밍됩니다. 어떤 MCP 도구가 호출되었는지, 어떤 SQL이 실행되었는지 확인할 수 있습니다.
{% endhint %}
